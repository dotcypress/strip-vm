...
fn test_ecall() {
  ...
  ",
    6,
    [0, 0, 0, 48271, 48271, 96542, 96542, 0],
    vec![0, 0, 0, 0, 0, 0, 0, 0],
  );
}
...
fn test_mem() {
  ...
  ",
    7,
    [0, 0, 0, 0xaf, 0xaf, -81, -20736, 44800],
...
fn test_multi_spin() {
  ...
  .unwrap();
  assert_eq!(pc, 3);
  assert_eq!(reg[..8], [0, 0, 0, -2, 2, 0, 0, 0,]);
...
fn assert_vm_state(code: &str, target_pc: usize, target_reg: [i32; 8], target_mem: Vec<u8>) {
  ...
  assert_eq!(reg[..8], target_reg);
  assert_eq!(ram, target_mem);
}

fn spin_vm(spins: u16, code: &str) -> Result<(usize, [i32; 32], Vec<u8>), VMError> {
